...
public class RegisterController extends MasterController{
	...
	public void register() {
		...
		if(id.isEmpty() || name.isEmpty() || strPass.isEmpty() || info.isEmpty()) {
			Util.showAlert("비어있음", "빈칸이 남아 있습니다.", AlertType.INFORMATION);
			return;
		}
		...
		ResultSet rs = null;
		String sqlExist = "SELECT * FROM Hi WHERE id = ?";
		String sqlInsert = "INSERT INTO Hi (`id`, `name`, `pass`, `info`, `money`, `lv`) "
							+ " VALUES (?, ?, PASSWORD(?), ?, 1000000, 0)";
		...
		try {
			...
			if(rs.next()) {
				//중복된 유저가 존재하는 거
				Util.showAlert("아이디 중복", "해당 아이디가 존재합니다.", AlertType.INFORMATION);
				return;				
			}
			JDBCUtil.close(rs);
